...
def plot_gunw_cli(argv: Sequence[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Create NISAR GUNW quick-look plots and diagnostic CSVs."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional GeoJSON polygon for crop/mask",
    )
    parser.add_argument(
        "--grid-epsg", type=int, default=None, help="EPSG code of GUNW x/y grid"
    )
    parser.add_argument(
        "--geojson-epsg", type=int, default=4326, help="EPSG code of input GeoJSON"
    )
    parser.add_argument(
        "--crop-padding", type=float, default=0.0, help="Crop padding in grid units"
    )
    parser.add_argument(
        "--no-mask-outside-geojson",
        action="store_true",
        help="Crop bbox but do not mask outside polygon",
    )
    parser.add_argument(
        "--pol", choices=["HH", "VV"], default=None, help="Force polarization"
    )
    parser.add_argument(
        "--radar-cube-index",
        type=int,
        default=0,
        help="Height index for radarGrid 3-D cube variables",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2200,
        help="Maximum plotted dimension after downsampling",
    )
    parser.add_argument(
