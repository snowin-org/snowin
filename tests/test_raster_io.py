...
def test_write_geotiff_roundtrip(tmp_path):
    ...
    out = write_geotiff(tmp_path / "a.tif", arr, np.array([10.0, 20.0]), np.array([100.0, 90.0]), 32611)
...
def test_write_geotiff_shape_mismatch(tmp_path):
    with pytest.raises(ValueError):
        write_geotiff(tmp_path / "a.tif", np.ones((2, 3)), np.array([1, 2]), np.array([3, 4]), 32611)
